...
- write a function that takes as input the output of the ODE solver and returns a summary
...
Upload your function to Canvas - InClass in this weeks Canvas
- we will give 10pts for all loaded functions
...
-   run ODE for each parameter sets to fill in this metrics data structure
    ...
    -   its usually helpful to create a wrapper function \* runs ODE \* extracts metrics
...
Lets first generate parameter sets and figure out how to run with across
uncertainty in just one  those parameter sets
...
## <span style="color:orange">Assignment</span> {.scrollable}
...
## <span style="color:orange">Your task</span> {.scrollable}
