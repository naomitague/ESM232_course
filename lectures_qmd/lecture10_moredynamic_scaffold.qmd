...
microperiods = c(5,50, 200)

# run the ode solver, using r = 0.05 (as defined above)
Ptime = ode(y=initialrabbits, times=microperiods, func=dexppop, parms=c(0.01))
colnames(Ptime)= c("year","Pop")
...
ggplot(as.data.frame(Ptime), aes(year, P))+geom_point()+labs(y="Pop",x="Years")
...
growth_result$Pode = tmp[,2]
# gather both model results using a key called "Ptype", value "P"

grnice = growth_result %>%  gather(key="method", value="P", -time)
# plot with model type as color

ggplot(grnice, aes(time, P, col=method))+geom_point()
