...
A city health department observe asthma hospitalization rates in a specific neighborhood hospital are twice as high as average rates in that state

An amphibian (salamander or newt) population has declined in the Santa Monica Mountains
...
e.g $Pop_newt = \beta_1 * \beta_2* precipitation + \beta_2*temperature +  \beta_2*presenceof predators + \beta_2*roaddensity + \beta_2*pollutionlevels$

So we need to think through relationships - what is connected with what

Example\
If we think the asthma hospitalization rates are influenced by emission from a new factory some distance away - we might have to consider relationship between the timing of emissions and prevailing wind patterns and how long particles stay in the air
...
[Assignment 2]("../assignments/Assignment2_ConceptualModel.qmd") 
