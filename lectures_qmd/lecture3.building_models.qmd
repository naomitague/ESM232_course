...
-   $solar$ is daily solar radiation (kJm$^{-2}/day$).
...
-   $ethresh$ threshold radiation below which efficiency declines linearly (10000 (kJ m$^{-2}$))
-   $eunits$ desired units for output default "kJ" Joules, option "kWhr" for Watts
-   $PR$ is the performance ratio (dimensionless, 0–1), accounting for site-specific losses (typically $\sim 0.75$)
