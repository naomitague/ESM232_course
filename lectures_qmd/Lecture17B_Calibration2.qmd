...
![Equifinality](img/weights.png)## Glue - generalized uncertainty analysis {.scrollable}
...
results[which.max(results$combined),"sim"]
```
...
What if all we care about is streamflow in August
(lets say fish are particularly sensitive to August flow when air temperature are warm)

What are our maximum likelihood estimates of august flow ?

Lets also compare, calibration using all metrics from class
and low flow calibration
...
msagel_month_avg$othercal = ifelse(msagel_month_avg$sim %in% other_cal_parms, "Accept Class", "Not By Class")
