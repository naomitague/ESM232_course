...
* include at least one "sub" metric that needs to be transformed
...
Using your performance metric

* Perform a split-sample calibration - you can decide what year to use for
calibration (its an experiment!)

* Find the best parameter set, and then graph something about streamflow (e.g daily, mean August, or?) for
the best parameter set

* Compute how the performance of the model using the best parameter set changed
in pre and post calibration periods

On the canvas survey - add the 'best' parameter set number (so we can how different metrics influence which parameter you pick)
...
# ad observed back
quant_flow = left_join(quant_flow, msage[,c("date","obs")], by=c("date"))
# format for plotting

quant_flowl = quant_flow %>% pivot_longer(col=c(-date), values_to="flow", names_to="quantile")

# we lost the dates
quant_flowl$year = as.integer(year(quant_flowl$date))
quant_flowl$monthn = as.character(month(quant_flowl$date))
quant_flowl$month = as.integer(month(quant_flowl$date))
quant_flowl$day = as.integer(day(quant_flowl$date))
quant_flowl$wy = ifelse(quant_flowl$month >= 10, quant_flowl$year+1, quant_flowl$year)
...
ggplot(subset(quant_flowl, wy==1980), aes(date, flow, col=quantile))+geom_line()+scale_y_continuous(trans="log")
...
To hand in - and Rmarkdown and R function

* R function that codes a metric for performance evaluation
  * must be a combination of at least two performance measures
  * include some comments that explain 'why' this metric
  
* R markdown that does the follow (with lots of documentation of the work flow)
  * applies your performance function to a part of the Sagehen data set (with multiple simulations) that you want to use for calibration
  * summarizes the performance over the calibration period in 1-2 graphs; you can decide what is useful 
  * uses the performance measure to select "acceptable" parameters
  * computes the range of the performance measure using only the "acceptable" paramete over the post-calibration period (part that you didn't use for calibration)
  * graph the range of output across acceptable parameters (e.g post-calibration parameter uncertainty); you can choose what output is most interesting for you
  * compute and graph the maximum likelihood estimate of your output of interest (e.g minimum summer streamflow each year) for the post-calibration period
