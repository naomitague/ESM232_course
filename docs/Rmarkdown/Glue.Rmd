...
qn = function(flow, wt) {
 qnr = wtd.quantile(flow, probs=c(0.1,0.5,0.9), weights=wt)
  return(qnr)}
 
# doesnt' work 
aver_flow = msagel_acc %>% group_by(date) %>% dplyr::summarize(
flow10=Quantile(x=flow, weight=wt_acc, probs=c(0.1,0.5,0.9))[1],
flow50=Quantile(x=flow, weight=wt_acc, probs=c(0.1,0.5,0.9))[2],
flow90=Quantile(x=flow, weight=wt_acc, probs=c(0.1,0.5,0.9))[3]
)

, 
flow10 = wtd.quantile(flow,probs=c(0.1), weights=wt_acc), flow90=wtd.quantile(flow, probs=c(0.90), weights=wt_acc))

# format for plotting
aver_flowl = aver_flow %>% pivot_longer(col=c(-date), values_to="flow", names_to="quantile")

# we lost the dates
aver_flowl$year = as.integer(year(aver_flowl$date))
aver_flowl$monthn = as.character(month(aver_flowl$date))
aver_flowl$month = as.integer(month(aver_flowl$date))
aver_flowl$day = as.integer(day(aver_flowl$date))
aver_flowl$wy = ifelse(aver_flowl$month >= 10, aver_flowl$year+1, aver_flowl$year)
ggplot(subset(aver_flowl, wy==1980), aes(date, flow, col=quantile))+geom_line()
